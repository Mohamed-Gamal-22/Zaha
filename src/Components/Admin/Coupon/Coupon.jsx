...
export default function Coupon() {
  ...
  async function getCoupon(page = 1) {
    let { data } = await axios.get(
      `https://www.zahascarves.com/api/coupon/all?page=${page}`,
      {
        headers: {
  ...
  async function addCoupon() {
    if (!discount || !price || !startDate || !endDate) {
      Swal.fire({
        icon: "warning",
    ...
    try {
      let { data } = await axios.post(
        `https://www.zahascarves.com/api/coupon/create`,
        formData,
        {
          headers: {
            Authorization: `Bearer tmTqMwqaJf0gGEQWE5kQAkfn37ITr46RpjVCfHWha266e4cc`,
  ...
  async function deleteCoupon(id) {
    try {
      let { data } = await axios.get(
        `https://www.zahascarves.com/api/coupon/delete/${id}`,
        {
          headers: {
            Authorization: `Bearer tmTqMwqaJf0gGEQWE5kQAkfn37ITr46RpjVCfHWha266e4cc`,
          },
  ...
  async function updateCoupon(id) {
    ...
    try {
      let { data } = await axios.post(
        `https://www.zahascarves.com/api/coupon/update/${id}`,
        {
          discount,
          price,
        },
      ...
      <>
        ...
        <Modal show={showUpdate} onHide={handleCloseUpdate}>
          ...
          <Modal.Footer>
            ...
            <Button
              variant="primary"
              onClick={() => updateCoupon(selectedCoupon.id)}
            >
              Update Coupon
            </Button>
          </Modal.Footer>
